...
class DynamicFormContainer extends _react.Component {
  constructor(props) {
    ...
    this.formatAdapterScheme = (0, _memoizeOne.default)((scheme, adapters) => adapters.reduce((res, adapter) => adapter(res), scheme), _lodash.default.isEqual);
  }
    ...
    function recursingApplay(scheme, parentKey) {
      ...
      scheme.map(({
        ...
        if (defaultValue && !_lodash.default.get(initialValues, formatKey(key), null)) {
          const adaptedValue = _lodash.default.reduce(specialDefaultValues.reverse(), (res, adapter) => adapter(defaultValue, type) || res, defaultValue);
    ...
    function rednderScheme(params) {
      ...
      return scheme.map(element => {
        ...
        if (filedsList[type]) {
          ...
          return _react.default.createElement(filedsList[type], (0, _extends2.default)({}, other, options, validation, filedsProps, {
            name: formatKey(key),
            validate: _formBuilderUtils.default.formatValidation(validation, type, formatKey(key))
          }));
        }
